---
title: "scott_foster_game_analysis"
output:
  html_document:
    df_print: paged
date: "2025-06-30"
...
data_summary = function(ref_data, min_games){
  ...
  foster_rank = sum(foster_trailing_team_win_pct < sub_data$trailing_team_win_pct)
...
First let's check if this hypothesis holds up. We'll grab the 10 most experienced referees in this time span and the 25 least experienced, and look at what percentage of games they officiate are in the first round:
...
And the winning percentage in later rounds is higher, as expected. Here we note that the winning percentage of trailing teams is higher when Foster is officiating. However, let's try to give this some scope and context.
...
Also, look at the sample sizes here: trailing teams are 45-43 with Foster. If we take 48% to be the "true" winning percentage we would expect, then after 88 games, the expected record would be 42-46. In other words, trailing teams only have 3 more wins than expected after nearly 100 games officiated by Foster. If he's truly the "Extender", then he's not very good at extending.
